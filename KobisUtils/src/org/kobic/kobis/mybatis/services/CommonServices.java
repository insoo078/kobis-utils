...
//	private HashMap<String, XCommonSheetObj> unmapped;
	...
	private boolean findKobisCodeInPhylogeneticTree( String speciesType, XCommonSheetObj commonSheetRecordObj, ExtraInfo extraInfo, List<PhylogeneticTreeVO> kobisPhylogeneListFromDB ) throws IntrospectionException, IllegalAccessException, IllegalArgumentException, InvocationTargetException {
		...
		for(Iterator<PhylogeneticTreeVO> iter=kobisPhylogeneListFromDB.iterator(); iter.hasNext();) {
			...
			if( extraPropertyValueFromDB.trim().toUpperCase().equals( extraInfo.extraValue(speciesType).trim().toUpperCase() ) ) {
...
//							if( speciesType.equals("var.") )		extraInfo = new ExtraInfo("var.",	seperateSpeciesToSpecific[1]);
//							else if( speciesType.equals("ssp.") )	extraInfo = new ExtraInfo("ssp.",	seperateSpeciesToSpecific[1]);
//							else if( speciesType.equals("f.") )		extraInfo = new ExtraInfo("f.",		seperateSpeciesToSpecific[1]);
//							else if( speciesType.equals("line") )	extraInfo = new ExtraInfo("line",	seperateSpeciesToSpecific[1]);
//							else { }
						}

						extraType = true;
						
						ret = this.findKobisCodeInPhylogeneticTree(speciesType, commonSheetRecordObj, extraInfo, kobisPhylogeneListFromDB);
					}else {
//						System.out.println( commonSheetRecordObj.getAccess_num() );
					}
					
					System.out.println("Processed " + ( j-3) + " / " + (this.getSheet().getLastRowNum() - 3 ) + " mapped (" + ret + ")" );
